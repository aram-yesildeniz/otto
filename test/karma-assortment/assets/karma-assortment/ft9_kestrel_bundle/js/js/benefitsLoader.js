...
export class Slide {
    constructor(container) {
        ...
        this.leverContainer = container.querySelector('.karma_assortment--benefits-container');
        this.ctaContainer = container.querySelector('.karma_assortment--cta-container');
...
export default class BenefitsLoader {
    ...
    init(carouselContainer, isCarousel = true) {
        const items = carouselContainer.querySelectorAll('.karma_assortment--item');
    ...
    appendLeverToSlide(lever, slide, isBufferSlide = false) {
        let node = lever.cloneNode(true);
        if (isBufferSlide) {
            let slideshow_lever = node.querySelector('.benefit_lever.benefit_lever_slideshow');
            slideshow_lever.setAttribute("data-tracked", true)
        }
        ...
        if (!isBufferSlide) {
            ...
            o_util.hardcore.executeInlineScripts(slide.leverContainer); // TODO Remove
        }
    }
            ...
            "method": "GET"
                ...
                if (response.status === 200) {
                    ...
                    if (response.getResponseHeader('x-aggregation') === 'tracking') {
                        ...
                        this.appendLeverToSlide(lever, slide, false);
